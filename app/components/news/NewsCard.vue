  ...
  <article
    ...
    <div
      ...
      <span
        ...
        >{{ getShortDate(news.createdAt) }}</span
      >
    </div>
    <div
      class="absolute -bottom-4 left-0 right-0 md:-bottom-10 md:left-6 md:right-6 bg-white rounded-2xl shadow-lg p-6 dark:bg-zinc-900">
      <NuxtLink :to="`/news/author/${news.userId}`">
        <div class="flex gap-2">
          <Icon class="bg-[#7EB693]" name="heroicons:user-16-solid" />
          <span>{{ `${news.user.firstName} ${news.user.lastName}` }}</span>
        </div>
      </NuxtLink>
      <div class="flex flex-col py-2">
        <h3 class="text-2xl font-bold">{{ news.title }}</h3>
        <p class="truncate md:overflow-visible md:whitespace-normal">
