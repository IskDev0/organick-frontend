...
<template>
  <section class="mt-40" v-if="newsInfo">
    ...
    <div class="container relative -top-32">
      <div
        class="flex flex-col-reverse gap-2 md:flex-col shadow-lg bg-white dark:bg-zinc-800 p-5 md:p-20 rounded-2xl">
        <div
          class="flex flex-col items-start md:flex-row md:items-center gap-4">
          <p>
            <span class="font-bold">Posted on:</span>
            {{ getDateFull(newsInfo.createdAt) }}
          </p>
          <div class="flex items-center gap-2">
            <Icon class="bg-[#7EB693]" name="heroicons:user-16-solid" />
            <span>
              <NuxtLink :to="`/news/author/${newsInfo.userId}`">
                By {{ newsInfo.user.firstName }} {{ newsInfo.user.lastName }}
              </NuxtLink>
            </span>
          </div>
        </div>
        <div class="flex flex-col gap-2">
          <h1 class="text-4xl md:text-7xl font-extrabold py-4">
            {{ newsInfo.title }}
          </h1>
          <p class="text-[#525C60] dark:text-gray-300">
            {{ newsInfo.shortDescription }}
          </p>
        </div>
      </div>
      <div
        v-html="newsInfo.content"
        class="htmlResult mt-20 prose dark:prose-invert" />
    </div>
  </section>
</template>
